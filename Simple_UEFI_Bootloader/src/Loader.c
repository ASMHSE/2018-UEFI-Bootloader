...
EFI_STATUS GoTime(EFI_HANDLE ImageHandle, GPU_CONFIG * Graphics, void *RSDPTable)
{
  ...
  // Open the kernel file from current drive root and point to it with KernelFile
	GoTimeStatus = CurrentDriveRoot->Open(CurrentDriveRoot, &KernelFile, L"PlotKern.efi", EFI_FILE_MODE_READ, EFI_FILE_READ_ONLY);
	if (EFI_ERROR(GoTimeStatus))
  {
...
//        ZeroMem(&AllocatedMemory, (pages << EFI_PAGE_SHIFT));
        ...
        if(VerifyZeroMem(pages << EFI_PAGE_SHIFT, AllocatedMemory))
        {
          ...
          else // Not our remains, proceed with discovery of viable memory address
          {
            ...
            // Verify it's empty
            while(((int)NewAddress != -1) && VerifyZeroMem(pages << EFI_PAGE_SHIFT, AllocatedMemory)) // Loop this in case the firmware is really screwed
            { // It's not empty :(
              ...
              { // Gotta keep looking for a good memory address
                ...
                while((GoTimeStatus != EFI_SUCCESS) && ((int) NewAddress != -1))
                {
                  if(GoTimeStatus == EFI_NOT_FOUND)
                  {
...
*/
  ...
  Loader_block->MemoryParams.Memory_Map = MemMap;
  Loader_block->MemoryParams.Memory_Map_Size = MemMapSize; 
  Loader_block->MemoryParams.Memory_Map_Descriptor_Size = MemMapDescriptorSize; 
  Loader_block->MemoryParams.Memory_Map_Descriptor_Version = MemMapDescriptorVersion;
